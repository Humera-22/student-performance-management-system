...
from app.dataclasses import (
    ...
    StudentResponse, CourseResponse, PerformanceResponse
)
from app.utils import Utils
from app.http_client import HttpClient
from app.auth.decorators import jwt_required, permission_required
...
@jwt_required
@permission_required(PERMISSIONS["STUDENT_VIEW"])
def get_students():
...
def create_student():
    ...
    Utils.redis_client.delete("students")
    return jsonify({"message": "Student created"}), 201


@api.route("/students/<int:id>", methods=["PUT"])
@jwt_required
@permission_required(PERMISSIONS["STUDENT_UPDATE"])
def update_student(id):
    ...
    if g.user.role == "student" and g.user.id != id:
...
@jwt_required
@permission_required(PERMISSIONS["STUDENT_DELETE"])
...
@jwt_required
@permission_required(PERMISSIONS["COURSE_VIEW"])
def get_courses():
...
@jwt_required
@permission_required(PERMISSIONS["COURSE_CREATE"])
def create_course():
...
@jwt_required
@permission_required(PERMISSIONS["PERFORMANCE_VIEW"])
def get_performance():
...
@jwt_required
@permission_required(PERMISSIONS["PERFORMANCE_ADD"])
def add_performance():
...
@jwt_required
@permission_required(PERMISSIONS["USER_CREATE"])
def create_user():
    data = request.get_json()
...
@jwt_required
@permission_required(PERMISSIONS["VIEW_EXTERNAL_INFO"])
def get_info():
...
def create_user():
    ...
    if not data.get("username") or not data.get("password") or not data.get("roles"):
        return jsonify({"error": "Username, password, and roles required"}), 400

    if User.query.filter_by(username=data["username"]).first():
        return jsonify({"error": "Username already exists"}), 409

    user = User(username=data["username"])
    user.roles = data["roles"]  # list of roles
    user.set_password(data["password"])
    db.session.add(user)
    db.session.commit()
    return jsonify({"message": "User created"}), 201
